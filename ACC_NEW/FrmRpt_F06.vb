Public Class FrmRpt_F06
    ...
    End Sub
    Private Sub CALLC()
        Dim MDWRITEOFF As String = " AND WRITEOFF=N'Y' "

        '===========   N'1. ສິນເຊື່ອປົກກະຕິ (A)'===============
        Dim A1 As String = "select count(*) as AA,BUSINESSTYPEDESC from AP_Loan where 1=1 " & MDWRITEOFF & " and LaonDate BETWEEN '" & Format(MdStartDate, "yyyy-MM-dd") & "' AND '" & Format(MdToDate, "yyyy-MM-dd") & "'  group by BUSINESSTYPEDESC order by BUSINESSTYPEDESC "
        Call LoadSqlData(A1, RSC)
        If RSC.RecordCount > 0 Then
            While Not RSC.EOF
                Dim K1 As String = "Update RPT_F06 set acc_Total=" & RSC.Fields("AA").Value & ",ACC=" & RSC.Fields("AA").Value & " where left(Description,3)=N'" & Microsoft.VisualBasic.Left(RSC.Fields("BUSINESSTYPEDESC").Value, 3) & "'  "
                CNN.Execute(K1)
                RSC.MoveNext()
            End While
        End If
        ' ====W===
        Dim W1 As String = "select count(*) as AA,BUSINESSTYPEDESC from AP_Loan where 1=1  " & MDWRITEOFF & " and LaonDate BETWEEN '" & Format(MdStartDate, "yyyy-MM-dd") & "' AND '" & Format(MdToDate, "yyyy-MM-dd") & "'  and  GENDER=N'F' group by BUSINESSTYPEDESC order by BUSINESSTYPEDESC "
        Call LoadSqlData(W1, RSC)
        If RSC.RecordCount > 0 Then
            While Not RSC.EOF
                Dim KW1 As String = "Update RPT_F06 set  ACC_W=" & RSC.Fields("AA").Value & " where left(Description,3)=N'" & Microsoft.VisualBasic.Left(RSC.Fields("BUSINESSTYPEDESC").Value, 3) & "'  "
                CNN.Execute(KW1)
                RSC.MoveNext()
            End While
        End If
        ' ====AMT KIP  TT===
        Dim AMT1 As String = "select isnull(sum(Principle_LAK),0) as AA,BUSINESSTYPEDESC from AP_Loan  where 1=1  " & MDWRITEOFF & " and LaonDate BETWEEN '" & Format(MdStartDate, "yyyy-MM-dd") & "' AND '" & Format(MdToDate, "yyyy-MM-dd") & "'   group by BUSINESSTYPEDESC order by BUSINESSTYPEDESC "
        Call LoadSqlData(AMT1, RSC)
        If RSC.RecordCount > 0 Then
            While Not RSC.EOF
                Dim KW1 As String = "Update RPT_F06 set  Amt_1=" & RSC.Fields("AA").Value & " where left(Description,3)=N'" & Microsoft.VisualBasic.Left(RSC.Fields("BUSINESSTYPEDESC").Value, 3) & "'  "
                CNN.Execute(KW1)
                RSC.MoveNext()
            End While
        End If
        ' ====AMT KIP  W===
        Dim AMT_W As String = "select isnull(sum(Principle_LAK),0) as AA,BUSINESSTYPEDESC from AP_Loan  where 1=1  " & MDWRITEOFF & " and LaonDate BETWEEN '" & Format(MdStartDate, "yyyy-MM-dd") & "' AND '" & Format(MdToDate, "yyyy-MM-dd") & "'   and GENDER=N'F' group by BUSINESSTYPEDESC order by BUSINESSTYPEDESC "
        Call LoadSqlData(AMT_W, RSC)
        If RSC.RecordCount > 0 Then
            While Not RSC.EOF
                Dim KW1 As String = "Update RPT_F06 set  Amt_2=" & RSC.Fields("AA").Value & " where left(Description,3)=N'" & Microsoft.VisualBasic.Left(RSC.Fields("BUSINESSTYPEDESC").Value, 3) & "'   "
                CNN.Execute(KW1)
                RSC.MoveNext()
            End While
        End If
        ...
        ' ====INT KIP  TT===
        Dim AMT2 As String = "select isnull(sum(Int_LAK),0) as AA,BUSINESSTYPEDESC from AP_Loan  where 1=1  " & MDWRITEOFF & " and LaonDate BETWEEN '" & Format(MdStartDate, "yyyy-MM-dd") & "' AND '" & Format(MdToDate, "yyyy-MM-dd") & "'  group by BUSINESSTYPEDESC order by BUSINESSTYPEDESC "
        Call LoadSqlData(AMT2, RSC)
        If RSC.RecordCount > 0 Then
            While Not RSC.EOF
                Dim KW1 As String = "Update RPT_F06 set  Int_1=" & RSC.Fields("AA").Value & " where left(Description,3)=N'" & Microsoft.VisualBasic.Left(RSC.Fields("BUSINESSTYPEDESC").Value, 3) & "' "
                CNN.Execute(KW1)
                RSC.MoveNext()
            End While
        End If
        ' ====INT KIP  W===
        Dim AMT_W2 As String = "select isnull(sum(Int_LAK),0) as AA,BUSINESSTYPEDESC from AP_Loan  where 1=1  " & MDWRITEOFF & " and LaonDate BETWEEN '" & Format(MdStartDate, "yyyy-MM-dd") & "' AND '" & Format(MdToDate, "yyyy-MM-dd") & "'  and GENDER=N'F' group by BUSINESSTYPEDESC order by BUSINESSTYPEDESC "
        Call LoadSqlData(AMT_W2, RSC)
        If RSC.RecordCount > 0 Then
            While Not RSC.EOF
                Dim KW1 As String = "Update RPT_F06 set  Int_2=" & RSC.Fields("AA").Value & " where left(Description,3)=N'" & Microsoft.VisualBasic.Left(RSC.Fields("BUSINESSTYPEDESC").Value, 3) & "'  "
                CNN.Execute(KW1)
                RSC.MoveNext()
            End While
        End If
    ...
    Private Sub INSERTHEADER()
        ...
        If RSC.RecordCount = 0 Then
            Dim k As String = "INSERT INTO Header(ID,Nm,S1,S2,S3,S4,S5,S6,PP) " & _
                          " values(N'F06',N'" & txtHeader.Text & "',N'" & txtSig1.Text & "',N'" & txtSig2.Text & "',N'" & txtSig3.Text & "',N'" & txtSig4.Text & "',N'" & txtSig5.Text & "',N'" & txtSig6.Text & "',N'" & (TxtPP.Text) & "') "
            CNN.Execute(k)
        Else
            CNN.Execute("UPDATE Header set Nm=N'" & txtHeader.Text & "',S1=N'" & txtSig1.Text & "',S2=N'" & txtSig2.Text & "',S3=N'" & txtSig3.Text & "',S4=N'" & txtSig4.Text & "',S5=N'" & txtSig5.Text & "',S6=N'" & txtSig6.Text & "',PP=N'" & (TxtPP.Text) & "' where ID='F06' ")
        End If
    End Sub
    Private Sub Button3_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Button3.Click
        INSERTHEADER()
        CNN.Execute("UPDATE RPT_F06 set Acc=0,Acc_Total=0,Acc_W=0,Amt_1=0,Amt_2=0,Int_1=0,Int_2=0  ")
        Call CALLC()
      
        Dim RsKK As New ADODB.Recordset
        With RsKK
            Dim PPP As String = "SELECT N'" & RmFrdate.Text & "' as DD,N'" & txtSig1.Text & "' as S1,N'" & txtSig2.Text & "' as S2,N'" & txtSig3.Text & "' as S3,N'" & txtSig4.Text & "' as S4,N'" & txtSig5.Text & "' as S5,N'" & txtSig6.Text & "' as S6,N'" & TxtPP.Text & "' as pp, * from RPT_F06 order by Rpt_ID ASC "
