Public Class FrmAsset_Grp
    ...
    Private Sub LoadDG()
        ...
        Try
            ConnectCL()
            SQL = "Select 0,Group_ID,Group_Nm,Group_NmE,AccountCodeAsDR,AccountCodeAsCR,Ac_Code,Dep_Code,AccountCodeBrokenDR,Grp_no from Groups  " & mSql & " "
            LoadCN_DG()
            da.Fill(ds, "Groups")
            DG.DataSource = ds.Tables("Groups")
            cn.Close()
    ...
    Private Sub Button4_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnDel.Click
        ...
        If MsgBox("ທ່ານຕ້ອງການລຶບລາຍການຊັບສິນເລກລະຫັດ " & myTemp & " ນີ້ບໍ່?", MsgBoxStyle.Critical + MsgBoxStyle.YesNo + MsgBoxStyle.DefaultButton2) = Windows.Forms.DialogResult.Yes Then
            CNN.Execute("DELETE FROM Groups WHERE Group_ID='" & myTemp & "' ")
        End If
        Call LoadDG()
    End Sub
    ...
    Private Sub Button3_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnPrint.Click
        ...
        Dim str As String = ""
        Call LoadSqlData("Select * from Groups Where 1=1" & mSql & "  ", Rs)
