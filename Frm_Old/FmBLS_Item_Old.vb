Public Class FmBLS_Item_Old
    Public Sub LoadListFG()
        ...
        With RSC
            ...
            Call LoadSqlData("SELECT * FROM  Ap_Rpt_BLS_Old  where 1=1 " & RPT_GRP & "  order by GRP, Rpt_ID ASC  ", RSC)
    ...
    Private Sub loadBankItem()
        ...
        With RSC
            ...
            Call LoadSqlData("SELECT * FROM  Ap_Rpt_BLS_Item_Old where Rpt_ID=N'" & TextBox1.Text & "'  " & RPT_GRP & " Order by Ac_Code ", RSC)
            If .RecordCount > 0 Then
                While Not .EOF
    ...
    Private Sub BtnEdit_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles BtnEdit.Click
        ...
        For i = 1 To FG.Rows - 1
            ...
            CNN.Execute("Update Ap_Rpt_BLS_Old Set  Description = N'" & Apostrophe(FG.get_TextMatrix(i, 2)) & "' ,  Descriptione = N'" & Apostrophe(FG.get_TextMatrix(i, 3)) & "' , Chart_of_Accounts_Codes = N'" & FG.get_TextMatrix(i, 4) & "'  Where Rpt_ID = '" & FG.get_TextMatrix(i, 1) & "'")
    ...
    Private Sub Button2_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Button2.Click
        MsgBox("ການບັນຶກສຳເລັດຜົນ")
        CNN.Execute("delete Ap_Rpt_BLS_Item_Old where Rpt_ID = '" & TextBox1.Text & "' ")
        Dim i As Integer
        For i = 1 To FG2.Rows - 1
            ...
            If FG2.get_TextMatrix(i, 1) = "" And FG2.get_TextMatrix(i, 2) = "" Then
                Exit Sub
            End If
            CNN.Execute("INSERT INTO Ap_Rpt_BLS_Item_Old( Rpt_ID,  Ac_Code , Ac_Name , Ac_NameE ,Amt_Dr , Amt_Cr , BLS  , Rpt_Type) " & _
    ...
    Private Sub AC_Code_KeyPress(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyPressEventArgs) Handles AC_Code.KeyPress
        If e.KeyChar = Chr(13) Then
            LoadSqlData("Select top 1 Rpt_ID , Ac_Code from Ap_Rpt_BLS_Item_Old where  Ac_Code like '" & AC_Code.Text & "%'  And Rpt_ID <> '" & RPT_ID.Text & "'  " & RPT_GRP & " ", RSC)
            If RSC.RecordCount <> 0 Then
                MsgBox("ເລກບັນຊີ " & Trim(CStr(RSC.Fields("Ac_Code").Value.ToString)) & " ມີຢູ່ " & Trim(CStr(RSC.Fields("Rpt_ID").Value.ToString)) & " ແລ້ວ")
                Exit Sub
            End If

            CNN.Execute("delete Ap_Rpt_BLS_Item_Old where Ac_Code like '" & AC_Code.Text & "%' And Rpt_ID = '" & RPT_ID.Text & "' And Rpt_Type = '" & Rpt_Type.Text & "'  " & RPT_GRP & " ")
            'Dim SS As String = " insert into Ap_Rpt_BLS_Item_Old (Rpt_ID , Ac_Code , Ac_Name, Rpt_Type, GRP) select N'" & RPT_ID.Text & "' ,  Ac_Code , Name_L , '" & Rpt_Type.Text & "',  " & RPT_GRPID & " from Acc_Code where Ac_Code like '" & AC_Code.Text & "%' and acc_type=N'ບັນຊີແມ່ (P)'   "
    ...
    Private Sub Button3_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Button3.Click
        CNN.Execute("delete Ap_Rpt_BLS_Item_Old where  Rpt_ID =N'" & RPT_ID.Text & "' " & RPT_GRP & "   ")
        TextBox1.Text = FG.get_TextMatrix(FG.Row, 1)
        Call loadBankItem()
    End Sub
    ...
    Private Sub Button6_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Button6.Click
        ...
        With RSC
            Call LoadSqlData("SELECT * FROM  Ap_Rpt_BLS_Item_Old  Order by Ac_Code  ", RSC)
            If .RecordCount > 0 Then
                While Not .EOF
                    CNN.Execute("delete Ap_Rpt_Item where Ac_Code like '" & Trim(CStr(.Fields("Ac_Code").Value.ToString)) & "%' And Rpt_ID = '" & Trim(CStr(.Fields("Rpt_ID").Value.ToString)) & "' And Rpt_Type = '" & Trim(CStr(.Fields("Rpt_Type").Value.ToString)) & "' " & _
                                " insert into Ap_Rpt_BLS_Item_Old (Rpt_ID , Ac_Code , Ac_Name, Rpt_Type) select '" & Trim(CStr(.Fields("Rpt_ID").Value.ToString)) & "' ,  Ac_Code , Name_L , '" & Trim(CStr(.Fields("Rpt_Type").Value.ToString)) & "' from Acc_Code where Ac_Code like '" & Trim(CStr(.Fields("Ac_Code").Value.ToString)) & "%'  ")
                    .MoveNext()
                End While
            Else
            End If
        End With
        CNN.Execute("delete Ap_Rpt_BLS_Item_Old")
        CNN.Execute(" insert into Ap_Rpt_BLS_Item_Old  (Rpt_ID , Ac_Code , Ac_Name, Rpt_Type) select Rpt_ID , Ac_Code , Ac_Name, Rpt_Type from Ap_Rpt_Item")
        MsgBox("Ok")
