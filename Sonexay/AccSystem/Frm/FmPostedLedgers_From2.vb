Public Class FmPostedLedgers_From2
    ...
    Private Sub BtnPreview_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles BtnPreview.Click
        ...
        Dim DD As String
        DD = "  update  Ap_PostedLedgers set Ap_PostedLedgers.ac_code_dr = gen_jn.code_dr from Ap_PostedLedgers,gen_jn " & _
      "  where Ap_PostedLedgers.certify = gen_jn.certify and gen_jn.code_dr <>''   "
        CNN.Execute(DD)

        DD = "   update  Ap_PostedLedgers set Ap_PostedLedgers.ac_code_cr = gen_jn.code_cr from Ap_PostedLedgers,gen_jn " & _
      "  where Ap_PostedLedgers.certify = gen_jn.certify and gen_jn.code_cr <>''   "
        CNN.Execute(DD)

        DD = "   update  Ap_PostedLedgers set Ac_code_dr='' where Ac_code_dr=Ac_code "
        CNN.Execute(DD)
        DD = "  update  Ap_PostedLedgers set Ac_code_cr='' where Ac_code_cr=Ac_code "
        CNN.Execute(DD)
        CNN.Execute(" update  Ap_PostedLedgers set amt_cr=amt_dr, amount_cr=amount_dr where Ac_code_cr<>'' ")
        CNN.Execute(" update  Ap_PostedLedgers set amt_dr=amt_cr, amount_dr=amount_cr where Ac_code_dr<>'' ")
        CNN.Execute(" update  Ap_PostedLedgers set amt_cr=0, amount_cr=0 where Ac_code_cr='' ")
        CNN.Execute(" update  Ap_PostedLedgers set amt_dr=0, amount_dr=0 where Ac_code_dr='' ")
        '================================================
        If MuLng = "L" Then
        ...
        Else
            CNN.Execute("update Ap_PostedLedgers set Ap_PostedLedgers.ac_name=gen_jn.ac_namee from Ap_PostedLedgers , gen_jn where Ap_PostedLedgers.Ac_Code = gen_jn.Ac_Code and   Ap_PostedLedgers.certify =gen_jn.certify ")
        ...
        If CheckBox1.Checked = True Then
            LngId = "7044" : CallLngStr() : MuLngRpt = MuLngRpt & "N'" & LngStr & " " & DTDATE02 & "' As Crl_RptName ,"
        Else
            LngId = "7067" : CallLngStr() : MuLngRpt = MuLngRpt & "N'" & LngStr & " " & DTDATE02 & "' As Crl_RptName ,"
        End If
        ...
        If CheckBox1.Checked = True Then
            Dim Rpt As New CryPostedLedgersCurr_From2
        ...
        Else
            ...
            If MuLng = "L" Then
                Rpt = New CryPostedLedgers_From2
            Else
                Rpt = New CryPostedLedgers_Eng_From2
            End If
