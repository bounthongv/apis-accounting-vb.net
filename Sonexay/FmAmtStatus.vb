Public Class FmAmtStatus
    ...
    Private Sub BtnPreview_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles BtnPreview.Click
        ...
        If MDACC00 = 0 Then
            If Month(MdStartDate) <> 1 Then
                ...
                If Month(MdStartDate) > 2 Then
                    Dim AA As String = "update AP_Rpt_Amt_Status set Amt4= Amt4+ (select sum(Amt_cr)-sum(Amt_dr) from gen_jn where   (left(ac_code,1)='4' or left(ac_code,1)='5')  and Month(Date_Work)<'" & Month(ds) - 1 & "' and Year(Date_Work)='" & Year(ds) & "' )  where rpt_id='01'"
        ...
        End If
                ...
                Dim KK As String = "update AP_Rpt_Amt_Status set Amt1=  (select (sum(amount_dr)-sum(amount_cr))* " & CDbl(MD_Rate) & " from Gen_jn where (Ac_Code Like '3108%' )  And Month(Date_Work)<='" & Month(ds) - 1 & "' And Year(Date_Work)='" & Year(ds) & "')  where rpt_id='15'"
                DbHelper.ExecuteNonQuery(KK)
        ...
        End If
                '========== - ຜົນກະທົບຈາກອັດຕາແລກປ່ຽນເງິນຕາຕ່າງປະເທດ
                Dim KK2 As String = "update AP_Rpt_Amt_Status set Amt1=Amt1-(select (sum(amount_dr)-sum(amount_cr))* " & CDbl(MD_Rate) & "  from Gen_jn where (Ac_Code Like '3108%' )  And Month(Date_Work)<='" & Month(ds) - 1 & "' And Year(Date_Work)='" & Year(ds) & "')  where rpt_id='15'"
            ...
            MMM22()
            ds = DateAdd(DateInterval.Year, 0, MdStartDate)
            DbHelper.ExecuteNonQuery("update AP_Rpt_Amt_Status set Amt1=  (select sum(Amt_Cr+ Amt_Dr) from Open_jn where Ac_Code Like '310%' And Year(Date_Work)='" & Year(ds) & "')  where rpt_id='16'")
    ...
    Private Sub LoadOpen_Jn16()
        Dim add As String = "select sum(rem_cr - rem_dr) as x  , count(rem_cr - rem_dr) as y  from Ap_balance_6_col  where Ac_Code Like '1%' or Ac_Code Like '2%' or Ac_Code Like '3%' "
    ...
    Private Sub UpdateIIn_Item(ByVal row As DataRow)
        Dim dtInner As DataTable = DbHelper.GetDataTable(" select * from Ap_balance_6_col   where ac_code =  '" & DbHelper.GetStr(row("Ac_Code")) & "' ")
        For Each innerRow As DataRow In dtInner.Rows
            DbHelper.ExecuteNonQuery("Insert into Ap_Rpt_Cashflow_Detail (  Rpt_Id , Ac_Code , Ac_Name , Last_Amt_Dr , Last_Amt_Cr , Amt_Dr , Amt_Cr , Rpt_Type ) values ( '" & DbHelper.GetStr(row("Rpt_Id")) & "' , '" & DbHelper.GetStr(innerRow("Ac_Code")) & "' , N'" & DbHelper.GetStr(innerRow("Ac_Name")) & "'   , " & CDbl(DbHelper.GetStr(innerRow("open_amt_dr"))) & " , " & CDbl(DbHelper.GetStr(innerRow("open_amt_Cr"))) & "   , " & CDbl(DbHelper.GetStr(innerRow("Rem_dr"))) & " , " & CDbl(DbHelper.GetStr(innerRow("Rem_cr"))) & " , 'In')")
    ...
    Private Sub UpdateIIn_ItemLast(ByVal row As DataRow)
        Dim dtInner As DataTable = DbHelper.GetDataTable(" select * from Ap_balance_6_col   where ac_code =  '" & DbHelper.GetStr(row("Ac_Code")) & "' ")
        For Each innerRow As DataRow In dtInner.Rows
            DbHelper.ExecuteNonQuery("Insert into Ap_Rpt_Cashflow_Detail (Ac_Code_Parent , Rpt_Id , Ac_Code , Ac_Name  , Last_Amt_Dr , Last_Amt_Cr , Rpt_Type) values (  '" & DbHelper.GetStr(row("Ac_Code")) & "' , '" & DbHelper.GetStr(row("Rpt_Id")) & "' , '" & DbHelper.GetStr(innerRow("Ac_Code")) & "' , N'" & DbHelper.GetStr(innerRow("Ac_Name")) & "'  , " & CDbl(DbHelper.GetStr(innerRow("Rem_dr"))) & " , " & CDbl(DbHelper.GetStr(innerRow("Rem_cr"))) & " , 'In' )")
            DbHelper.ExecuteNonQuery("update  Ap_Rpt_Cashflow_Item set Amt_Dr  =  Amt_Dr+" & CDbl(DbHelper.GetStr(innerRow("Rem_dr"))) & " , Amt_Cr  = Amt_Cr+" & CDbl(DbHelper.GetStr(innerRow("Rem_cr"))) & "   where ac_code = '" & DbHelper.GetStr(row("ac_code")) & "' And  Rpt_Type = 'In' ")
        Next
    End Sub
    ...
    Private Sub UpdateOut_Item(ByVal row As DataRow)
        Dim dtInner As DataTable = DbHelper.GetDataTable(" select * from Ap_balance_6_col   where ac_code =  '" & DbHelper.GetStr(row("Ac_Code")) & "' ")
        For Each innerRow As DataRow In dtInner.Rows
            DbHelper.ExecuteNonQuery("Insert into Ap_Rpt_Cashflow_Detail (Ac_Code_Parent , Rpt_Id , Ac_Code , Ac_Name  ,  Last_Amt_Dr , Last_Amt_Cr, Amt_Dr , Amt_Cr , Rpt_Type ) values (  '" & DbHelper.GetStr(row("Ac_Code")) & "' , '" & DbHelper.GetStr(row("Rpt_Id")) & "' , '" & DbHelper.GetStr(innerRow("Ac_Code")) & "' , N'" & DbHelper.GetStr(innerRow("Ac_Name")) & "'  ,   " & CDbl(DbHelper.GetStr(innerRow("Open_Amt_dr"))) & " , " & CDbl(DbHelper.GetStr(innerRow("Open_Amt_cr"))) & " , " & CDbl(DbHelper.GetStr(innerRow("Rem_dr"))) & " , " & CDbl(DbHelper.GetStr(innerRow("Rem_cr"))) & " , 'Out' )")
    ...
    Private Sub UpdateOut_ItemLast(ByVal row As DataRow)
        ...
        For Each innerRow As DataRow In dtInner.Rows
            DbHelper.ExecuteNonQuery("Insert into Ap_Rpt_Cashflow_Detail (Ac_Code_Parent , Rpt_Id , Ac_Code , Ac_Name  , Last_Amt_Dr , Last_Amt_Cr , Rpt_Type) values (  '" & DbHelper.GetStr(row("Ac_Code")) & "' , '" & DbHelper.GetStr(row("Rpt_Id")) & "' , '" & DbHelper.GetStr(innerRow("Ac_Code")) & "' , N'" & DbHelper.GetStr(innerRow("Ac_Name")) & "'  , " & CDbl(DbHelper.GetStr(innerRow("Rem_dr"))) & " , " & CDbl(DbHelper.GetStr(innerRow("Rem_cr"))) & ", 'Out')")
            DbHelper.ExecuteNonQuery("update  Ap_Rpt_Cashflow_Item set Amt_Dr  =  Amt_Dr+" & CDbl(DbHelper.GetStr(innerRow("Rem_dr"))) & " , Amt_Cr  = Amt_Cr+" & CDbl(DbHelper.GetStr(innerRow("Rem_cr"))) & "   where ac_code = '" & DbHelper.GetStr(row("ac_code")) & "' And  Rpt_Type = 'Out' ")
        Next
    End Sub
        ...
        If dt.Rows.Count > 0 Then
            For Each row As DataRow In dt.Rows
                ...
                If DbHelper.GetStr(row("lck")) = "1" Then
                    'DbHelper.ExecuteNonQuery(" Update  Ap_Rpt_Cashflow set Amt = " & CLT_Str & " , Last_Amt = " & CLT_Last_Str & " where  Rpt_ID =   '" & DbHelper.GetStr(row("Rpt_ID")) & "' ")
                    CLT_Str = ""
                    CLT_Last_Str = ""
                End If
            Next
    ...
    Private Sub LoadOpen_Jn14(ByVal row As DataRow)
        Dim dtInner As DataTable = DbHelper.GetDataTable("   select * from Ap_balance_6  where ac_code = '" & DbHelper.GetStr(row("ac_code")) & "'  ")

        If dtInner.Rows.Count <> 0 Then
            DbHelper.ExecuteNonQuery("Update Ap_balance_6_col set Quarter_dr= " & CDbl(Trim(DbHelper.GetStr(row("amt_dr")))) & "  , Quarter_cr= " & CDbl(Trim(DbHelper.GetStr(row("amt_cr")))) & "  where ac_code = '" & DbHelper.GetStr(row("ac_code")) & "'  ")
        Else
            MsgBox("ggg")
