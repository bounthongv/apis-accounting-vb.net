Module MuHideMenu
    ...
    Public Sub HideMenu()
        If MUserID <> "a" Then
            LoadSqlData("select Menu_Nm  from  Ap_Section_Item where Ints = 0  And Usr_Id= '" & MUserID & "' Order by Menu_Nm", RSC)
            With RSC
                Do Until .EOF = True
                    MuHide = (.Fields("Menu_Nm").Value)
                    Call LoadMenuItem()
                    .MoveNext()
                Loop
            End With
    ...
    Public Sub InsertMenuStrip1()
        ...
        NM2 = 0
        NM3 = 0
        CNN.Execute("Delete Ap_Section   Delete Ap_Section_AdNew Delete Ap_Section_Item ")
        Dim N As Integer = 0
        For i = 0 To FmMain.MenuStrip1.Items.Count - 1
            N = N + 1
            ...
            End If
        Next i
        LoadSqlData("select Usr_id  from  Ap_Users  Order by Usr_id", RSC)
        With RSC
            Do Until .EOF = True
                Usr = (.Fields("Usr_id").Value)
                LoadUsrACC()
                .MoveNext()
            Loop
        End With
        ...
        CNN.Execute("delete Ap_Section_Item")
        ...
        CNN.Execute("Update Ap_Section_Item set Ints = 1 where  Usr_Id = 'a'  ")
        CNN.Execute("Update Ap_Section_Item set Ints = 1 ")
    End Sub
    ...
    Private Sub LoadUsrACC()
        ...
        For i = 0 To FmMain.MenuStrip1.Items.Count - 1
            ...
            mItem = CType(FmMain.MenuStrip1.Items.Item(i), ToolStripMenuItem)
            mItem = CType(FmMain.MenuStrip1.Items.Item(i), ToolStripMenuItem)
            CNN.Execute("Insert Into Ap_Section_AdNew (Sec_ID , Sec_Nm ,Ints, Menu_Nm , Usr_Id) VAlues (" & N & " ,N'" & N & " " & FmMain.MenuStrip1.Items(i).Text & "'   ,1,N'" & FmMain.MenuStrip1.Items(i).Name & "' , '" & Usr & "' )")
            For j = 0 To mItem.DropDownItems.Count - 1
                'MsgBox(mItem.DropDownItems(j).Text)
                If mItem.DropDownItems(j).Text <> "" Then
                    CNN.Execute("Insert Into Ap_Section_AdNew (Sec_ID , Sec_Nm ,Ints, Menu_Nm , Usr_Id) VAlues (" & N & " ,N'*  " & mItem.DropDownItems(j).Text & "'   ,0,N'" & mItem.DropDownItems(j).Name & "' , '" & Usr & "' )")

                Else
                    CNN.Execute("Insert Into Ap_Section_AdNew (Sec_ID , Sec_Nm ,Ints, Menu_Nm , Usr_Id) VAlues (" & N & " ,N'----------------------------------------------------'   ,0,N'" & mItem.DropDownItems(j).Name & "' , '" & Usr & "' )")
                ...
                If mItem.DropDownItems(j).Text <> "" Then
                    ...
                    For a = 0 To mSubItem.DropDownItems.Count - 1
                        ...
                        If mSubItem.DropDownItems(a).Text <> "" Then
                            CNN.Execute("Insert Into Ap_Section_AdNew (Sec_ID , Sec_Nm ,Ints, Menu_Nm , Usr_Id) VAlues (" & N & " ,N' - " & mSubItem.DropDownItems(a).Text & "'   ,0,N'" & mSubItem.DropDownItems(a).Name & "' , '" & Usr & "' )")
                        Else
                            CNN.Execute("Insert Into Ap_Section_AdNew (Sec_ID , Sec_Nm ,Ints, Menu_Nm , Usr_Id) VAlues (" & N & " ,N'   ------------------------------------------------'   ,0,N'" & mSubItem.DropDownItems(a).Name & "' , '" & Usr & "' )")
