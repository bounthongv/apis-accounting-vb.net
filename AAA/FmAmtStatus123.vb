Public Class FmAmtStatus1234
   ...
   "  Last_Amt_Cr =Ap_balance_6_col.open_amt_cr,Amt_Dr =Ap_balance_6_col.amt_dr ,Amt_Cr =Ap_balance_6_col.amt_cr  " & _
    ...
    Private Sub BtnPreview_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles BtnPreview.Click
        ...
        DbHelper.ExecuteNonQuery("update AP_Rpt_Amt_Status set Amt1=0 ,amt2=0 , Amt3=0 , Amt4=0  ,Amt5=0")
        Dim ds As Date
        ...
        DbHelper.ExecuteNonQuery("update AP_Rpt_Amt_Status set Amt4=  Amt4+(select sum(Amt_Cr- Amt_Dr) from Open_jn where    Ac_Code Like '3908000%'    And Year(Date_Work)='" & Year(ds) & "')  where rpt_id='01'")

        DbHelper.ExecuteNonQuery("update AP_Rpt_Amt_Status set Amt4=  (select sum(Amt_dr) from Gen_jn where Ac_Code Like '3810000%'  And  Year(Date_Work)='" & Year(ds) & "') * -1 where rpt_id='03'")
        DbHelper.ExecuteNonQuery("update AP_Rpt_Amt_Status set Amt2=  (select sum(Amt_cr-Amt_Dr) from Gen_jn where Ac_Code Like '3202000%'  And  Year(Date_Work)='" & Year(ds) & "')  where rpt_id='04'")
        DbHelper.ExecuteNonQuery("update AP_Rpt_Amt_Status set Amt4=  (select sum(Amt_cr-Amt_Dr) from Gen_jn where Ac_Code Like '3202000%' And  Year(Date_Work)='" & Year(ds) & "') * -1 where rpt_id='04'")
        ...
        DbHelper.ExecuteNonQuery("update AP_Rpt_Amt_Status set Amt4=  (select sum(Amt_Dr) from Gen_jn where Ac_Code Like '3901000%' And Year(Date_Work)='" & Year(ds) & "')  where rpt_id='07'")
        ...
        DbHelper.ExecuteNonQuery("update AP_Rpt_Amt_Status set Amt2=  (select sum(Amt2) from AP_Rpt_Amt_Status  where rpt_id='01' Or rpt_id='02'  Or rpt_id='03' Or rpt_id='04' Or rpt_id='05' Or rpt_id='06' Or rpt_id='07' )  where rpt_id='08'")
        ...
        DbHelper.ExecuteNonQuery("update AP_Rpt_Amt_Status set Amt2=  (select sum(Amt2) from AP_Rpt_Amt_Status  where rpt_id='08' )  where rpt_id='10'")
        ...
        DbHelper.ExecuteNonQuery("update AP_Rpt_Amt_Status set Amt3=  (select sum(Amt_Dr) from Open_jn where Ac_Code Like '320%'  And Ac_Code<> '3202' And Year(Date_Work)=" & Year(ds) & ")  where rpt_id='12'")
        ...
        DbHelper.ExecuteNonQuery("update AP_Rpt_Amt_Status set Amt2=  (select sum(Amt_cr-Amt_Dr) from Gen_jn where Ac_Code Like '3202000%'   And date_work   BETWEEN '" & Format(MdStartDate, "yyyy-MM-dd") & "' AND '" & Format(MdToDate, "yyyy-MM-dd") & "')   where rpt_id='15'")
        DbHelper.ExecuteNonQuery("update AP_Rpt_Amt_Status set Amt4=  (select sum(Amt_cr-Amt_Dr) from Gen_jn where Ac_Code Like '3202000%'  And date_work   BETWEEN '" & Format(MdStartDate, "yyyy-MM-dd") & "' AND '" & Format(MdToDate, "yyyy-MM-dd") & "')  * -1 where rpt_id='15'")
        ...
        DbHelper.ExecuteNonQuery("update AP_Rpt_Amt_Status set Amt4=  (select sum(Amt4) from AP_Rpt_Amt_Status  where  rpt_id='12'   Or rpt_id='18')  where rpt_id='19'")
    ...
    Private Sub bar_neung()
        ...
        If RSC.RecordCount > 0 Then
            ...
            If CDbl(ram_dr) + CDbl(ram_cr) = 0 Then
                ...
                amt = amt_5 - amt_4
                DbHelper.ExecuteNonQuery("Update  Ap_balance_6_col set    amt_dr = " & CDbl(amt) & "   where ac_code='3901000' ")
                'DbHelper.ExecuteNonQuery("Update  Ap_balance_6_col set Rem_cr=0 , Rem_dr= ( amt_dr) - ( amt_cr) where ( amt_dr) >= (  amt_cr) and     ac_code='3901000' ")
                'DbHelper.ExecuteNonQuery("Update  Ap_balance_6_col set Rem_dr=0 , Rem_cr= (  amt_cr) - (  amt_dr) where ( amt_cr) >= (  amt_dr) and   ac_code='3901000' ")
                DbHelper.ExecuteNonQuery("Update  Ap_balance_6_col set Rem_cr=0 , Rem_dr= (open_amt_dr +  amt_dr) - (open_amt_cr +  amt_cr) where (open_amt_dr + amt_dr) >= (open_amt_cr +  amt_cr) ")
                DbHelper.ExecuteNonQuery("Update  Ap_balance_6_col set Rem_dr=0 , Rem_cr= (open_amt_cr +  amt_cr) - (open_amt_dr +  amt_dr) where (open_amt_cr +  amt_cr) >= (open_amt_dr +  amt_dr) ")

                'If amt > 0 Then
                '    DbHelper.ExecuteNonQuery("Update  Ap_balance_6_col set    amt_dr = " & CDbl(amt) & "   where'3901000' ")
                'Else
                '    DbHelper.ExecuteNonQuery("Update  Ap_balance_6_col set    amt_cr = " & CDbl(amt) & "   where'3901000' ")
                'End If
    ...
    Private Sub UpdateIIn_Item()
        ...
        With RSCkk
            Do Until .EOF = True
                DbHelper.ExecuteNonQuery("Insert into Ap_Rpt_Cashflow_Detail (  Rpt_Id , Ac_Code , Ac_Name , Last_Amt_Dr , Last_Amt_Cr , Amt_Dr , Amt_Cr , Rpt_Type ) values ( '" & CStr((RSCIn_M.Fields("Rpt_Id").Value)) & "' , '" & CStr((.Fields("Ac_Code").Value)) & "' , N'" & CStr((.Fields("Ac_Name").Value)) & "'   , " & CDbl((.Fields("open_amt_dr").Value)) & " , " & CDbl((.Fields("open_amt_Cr").Value)) & "   , " & CDbl((.Fields("Rem_dr").Value)) & " , " & CDbl((.Fields("Rem_cr").Value)) & " , 'In')")
                DbHelper.ExecuteNonQuery("update  Ap_Rpt_Cashflow_Item set  Last_amt_dr  =  Last_amt_dr+" & CDbl((.Fields("open_amt_dr").Value)) & " , Last_amt_cr  = Last_amt_cr+" & CDbl((.Fields("open_amt_Cr").Value)) & " , Amt_Dr  =  Amt_Dr+" & CDbl((.Fields("Rem_dr").Value)) & " , Amt_Cr  = Amt_Cr+" & CDbl((.Fields("Rem_cr").Value)) & "   where ac_code = '" & (RSCIn_M.Fields("ac_code").Value) & "' And  Rpt_Type = 'In' ")
    ...
    Private Sub UpdateOut_Item()
        ...
        With RSCkk
            Do Until .EOF = True
                ...
                DbHelper.ExecuteNonQuery("Insert into Ap_Rpt_Incon_Detail (Ac_Code_Parent , Rpt_Id , Ac_Code , Ac_Name  ,  Last_Amt_Dr , Last_Amt_Cr, Amt_Dr , Amt_Cr , Rpt_Type ) values (  '" & CStr((RSCIn_M.Fields("Ac_Code").Value)) & "' , '" & CStr((RSCIn_M.Fields("Rpt_Id").Value)) & "' , '" & CStr((.Fields("Ac_Code").Value)) & "' , N'" & CStr((.Fields("Ac_Name").Value)) & "'  ,   " & CDbl((.Fields("Open_Amt_dr").Value)) & " , " & CDbl((.Fields("Open_Amt_cr").Value)) & " , " & CDbl((.Fields("Amt_dr").Value)) & " , " & CDbl((.Fields("Amt_cr").Value)) & " , 'Out' )")
    ...
    Private Sub UpdateOut_ItemLast()
        ...
        With RSCkk
            Do Until .EOF = True
                DbHelper.ExecuteNonQuery("Insert into Ap_Rpt_Cashflow_Detail (Ac_Code_Parent , Rpt_Id , Ac_Code , Ac_Name  , Last_Amt_Dr , Last_Amt_Cr , Rpt_Type) values (  '" & CStr((RSCIn_M.Fields("Ac_Code").Value)) & "' , '" & CStr((RSCIn_M.Fields("Rpt_Id").Value)) & "' , '" & CStr((.Fields("Ac_Code").Value)) & "' , N'" & CStr((.Fields("Ac_Name").Value)) & "'  , " & CDbl((.Fields("Rem_dr").Value)) & " , " & CDbl((.Fields("Rem_cr").Value)) & ", 'Out')")
                DbHelper.ExecuteNonQuery("update  Ap_Rpt_Cashflow_Item set Amt_Dr  =  Amt_Dr+" & CDbl((.Fields("Rem_dr").Value)) & " , Amt_Cr  = Amt_Cr+" & CDbl((.Fields("Rem_cr").Value)) & "   where ac_code = '" & (RSCIn_M.Fields("ac_code").Value) & "' And  Rpt_Type = 'Out' ")
                .MoveNext()
            Loop
        End With
    ...
    Private Sub LoadOpen_Jn6()
        ...
        With RSC6
            Do Until .EOF = True
                ...
                '    DbHelper.ExecuteNonQuery("delete Ap_balance_6_col  where Accode='" & (.Fields("Ac_Code").Value) & "'")
                'End If
                .MoveNext()
            Loop
        End With
    ...
    Private Sub UpdateData2()
        ...
        With RSC2
            Do Until .EOF = True
                ...
                'MsgBox(amt_cr)
                If CDbl(op_dr + amt_dr) - CDbl(op_cr + amt_cr) >= 0 Then
                    DbHelper.ExecuteNonQuery("Update Ap_Rpt_Cashflow_Item set amt_dr='" & CDbl(op_dr + amt_dr) - CDbl(op_cr + amt_cr) & "' , amt_cr='" & CDbl(0) & "' where Accode='" & (.Fields("Ac_Code").Value) & "'")
                End If
                If CDbl(op_cr + amt_cr) - CDbl(op_dr + amt_dr) >= 0 Then
