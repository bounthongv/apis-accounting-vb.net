...
" from Ap_balance_6_col where left(ac_code,7)='2382120' group by  left(ac_code,7) ")
    ...
    Private Sub BtnPreview_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles BtnPreview.Click
        ...
        If CheckBox1.Checked = False Then
            ...
            If RP.Checked = True Then
                ...
                '        CNN.Execute("INSERT INTO TEST_ABC(amt) values('02') ")

                '        CNN.Execute("update  TEST_ABC set amt=(select  select sum(amt_dr),sum(amt_cr) from gen_jn   where (left(ac_code,1)='4' or left(ac_code,1)='5' )  and date_work   BETWEEN '2021-10-01' and  '2021-12-31'")
                ...
                CNN.Execute("   update Ap_Rpt_Cashflow set Ap_Rpt_Cashflow.Last_amt=(select   amt  from Ap_Rpt_BLS_Old where Rpt_Id='4.1.6') where Rpt_Id='02'")

                If Period.SelectedIndex = 3 Then
                    Dim PPP3 As String = "update Ap_Rpt_Cashflow set  amt=(   select sum(amt_dr)-sum(amt_cr) from gen_jn   where (left(ac_code,1)='4' or left(ac_code,1)='5' )  and date_work BETWEEN '" & Format(MdStartDate, "yyyy-MM-dd") & "' AND '" & Format(MdToDate, "yyyy-MM-dd") & "'  " & MULook2 & " ) where Rpt_Id='02'"
                    CNN.Execute(PPP3)
                End If
            ...
            '=========43	ເງິນສົດ ແລະ ທຽບເທົ່າເງິນສົດຍັງເຫຼືອທ້າຍເດືອນກ່ອນ====
            Dim MI43Last_amt As String = "delete AP_Sum "
            MI43Last_amt = MI43Last_amt & "  insert into AP_Sum(Rpt_ID,amt2,amt3) select '43' ,0,0 from Ap_Rpt_Cashflow     where rpt_ID='43'  "
            MI43Last_amt = MI43Last_amt & "  insert into AP_Sum(Rpt_ID,amt1,amt2) select  '111'  ,0 ,Last_amt from Ap_Rpt_Cashflow     where rpt_ID='42'   "
            MI43Last_amt = MI43Last_amt & "   insert into AP_Sum(Rpt_ID,amt2,amt3) select  '111'  ,0 ,Last_amt from Ap_Rpt_Cashflow     where rpt_ID='44'  "
            MI43Last_amt = MI43Last_amt & "  update AP_Sum set amt=(select sum(amt3)-sum(amt2) from AP_Sum where  (rpt_ID='111' or rpt_ID='111')  ) where rpt_ID='43'    "
            MI43Last_amt = MI43Last_amt & "  delete  AP_Sum  where rpt_ID='111'   "
            MI43Last_amt = MI43Last_amt & " Update Ap_Rpt_Cashflow set Ap_Rpt_Cashflow.Last_amt=AP_Sum.amt from AP_Sum,Ap_Rpt_Cashflow    where Ap_Rpt_Cashflow.rpt_ID=AP_Sum.rpt_ID "
            CNN.Execute(MI43Last_amt)


            If RM.Checked = True Then
                If Month(MdStartDate) > 1 Then
                    CNN.Execute("update Ap_Rpt_Cashflow set Ap_Rpt_Cashflow.amt=(select  Last_amt from Ap_Rpt_Cashflow where Rpt_Id='44') where Rpt_Id='43' ")
                    CNN.Execute("update Ap_Rpt_Cashflow set Description=N'ເງິນສົດ ແລະ ທຽບເທົ່າເງິນສົດຍັງເຫຼືອທ້າຍເດືອນກ່ອນ' where Rpt_Id='43' ")
                    CNN.Execute("update Ap_Rpt_Cashflow set Descriptione=N'Cash and cash equivalents at beginning of the month' where Rpt_Id='43' ")
                    CNN.Execute("update Ap_Rpt_Cashflow set Description=N'ເງິນສົດ ແລະ ທຽບເທົ່າເງິນສົດຍັງເຫຼືອທ້າຍເດືອນນີ້' where Rpt_Id='44' ")
                    CNN.Execute("update Ap_Rpt_Cashflow set Descriptione=N'Cash and cash equivalents at end of the month' where Rpt_Id='44' ")
                Else
                    'CNN.Execute("update Ap_Rpt_Cashflow set Ap_Rpt_Cashflow.amt=(select  Last_amt from Ap_Rpt_Cashflow where Rpt_Id='44') where Rpt_Id='43' ")

                    CNN.Execute("update Ap_Rpt_Cashflow set Description=N'ເງິນສົດ ແລະ ທຽບເທົ່າເງິນສົດຍັງເຫຼືອທ້າຍປີກ່ອນ' where Rpt_Id='43' ")
                    CNN.Execute("update Ap_Rpt_Cashflow set Descriptione=N'Cash and cash equivalents at beginning of the year' where Rpt_Id='43' ")
                    CNN.Execute("update Ap_Rpt_Cashflow set Description=N'ເງິນສົດ ແລະ ທຽບເທົ່າເງິນສົດຍັງເຫຼືອທ້າຍເດືອນນີ້' where Rpt_Id='44' ")
                    CNN.Execute("update Ap_Rpt_Cashflow set Descriptione=N'Cash and cash equivalents at end of the year' where Rpt_Id='44' ")
                End If
            ElseIf RP.Checked = True Then
                If Period.SelectedIndex = 0 Then
                    'CNN.Execute("update Ap_Rpt_Cashflow set Ap_Rpt_Cashflow.amt=(select  Last_amt from Ap_Rpt_Cashflow where Rpt_Id='44') where Rpt_Id='43' ")
                    CNN.Execute("update Ap_Rpt_Cashflow set Description=N'ເງິນສົດ ແລະ ທຽບເທົ່າເງິນສົດຍັງເຫຼືອທ້າຍງວດກ່ອນ' where Rpt_Id='43' ")
                    CNN.Execute("update Ap_Rpt_Cashflow set Descriptione=N'Cash and cash equivalents at beginning of the Period' where Rpt_Id='43' ")
                    CNN.Execute("update Ap_Rpt_Cashflow set Description=N'ເງິນສົດ ແລະ ທຽບເທົ່າເງິນສົດຍັງເຫຼືອທ້າຍງວດນີ້' where Rpt_Id='44' ")
                    CNN.Execute("update Ap_Rpt_Cashflow set Descriptione=N'Cash and cash equivalents at end of the Period' where Rpt_Id='44' ")
                Else
                    CNN.Execute("update Ap_Rpt_Cashflow set Ap_Rpt_Cashflow.amt=(select  Last_amt from Ap_Rpt_Cashflow where Rpt_Id='44') where Rpt_Id='43' ")
                    CNN.Execute("update Ap_Rpt_Cashflow set Description=N'ເງິນສົດ ແລະ ທຽບເທົ່າເງິນສົດຍັງເຫຼືອທ້າຍງວດກ່ອນ' where Rpt_Id='43' ")
                    CNN.Execute("update Ap_Rpt_Cashflow set Descriptione=N'Cash and cash equivalents at beginning of the Period' where Rpt_Id='43' ")
                    CNN.Execute("update Ap_Rpt_Cashflow set Description=N'ເງິນສົດ ແລະ ທຽບເທົ່າເງິນສົດຍັງເຫຼືອທ້າຍງວດນນີ້' where Rpt_Id='44' ")
                    CNN.Execute("update Ap_Rpt_Cashflow set Descriptione=N'Cash and cash equivalents at end of the Period' where Rpt_Id='44' ")
                End If
            Else
                CNN.Execute("update Ap_Rpt_Cashflow set Description=N'ເງິນສົດ ແລະ ທຽບເທົ່າເງິນສົດຍັງເຫຼືອທ້າຍປີກ່ອນ' where Rpt_Id='43' ")
                CNN.Execute("update Ap_Rpt_Cashflow set Descriptione=N'Cash and cash equivalents at beginning of the year' where Rpt_Id='43' ")
                CNN.Execute("update Ap_Rpt_Cashflow set Description=N'ເງິນສົດ ແລະ ທຽບເທົ່າເງິນສົດຍັງເຫຼືອທ້າຍປີນີ້' where Rpt_Id='44' ")
                CNN.Execute("update Ap_Rpt_Cashflow set Descriptione=N'Cash and cash equivalents at end of the year' where Rpt_Id='44' ")
            End If
            'CaCashflow()
            Call LoadReport()
  ...
  " select ac_code  ,  0 , 0  , 0  , 0  , sum(amount_Dr)* " & CDbl(txtRate.Text) & "  as amt_dr  , sum(amount_Cr)* " & CDbl(txtRate.Text) & "  as amt_Cr   from gen_jn  WHERE   curr='USD' and gen_jn.date_work   BETWEEN '" & Format(MdStartDate, "yyyy-MM-dd") & "' AND '" & Format(MdToDate, "yyyy-MM-dd") & "' " & Ac_Code & " " & MULook2 & "  group BY ac_code"
    ...
    " select ac_code  , sum(amount_Dr)* " & CDbl(txtRate.Text) & "  as amt_dr ,  sum(amount_Cr)* " & CDbl(txtRate.Text) & "  as amt_Cr ,  0 , 0  , 0 , 0  from Open_jn WHERE   curr='USD' and     date_work='" & "1-1-" & Format(MdStartDate, "yyyy") & "' " & Ac_Code & " " & MULook2 & "   group BY ac_code")
        ...
        CNN.Execute("update Ap_Rpt_Cashflow set Last_Amt = 0 , Amt  =  0 where   rpt_id<>'02'   ")
        ...
        CNN.Execute("update Ap_Rpt_Cashflow set Ap_Rpt_Cashflow.amt=(select   amt from Ap_Rpt_BLS_Old where Rpt_Id='4.1.6') where Rpt_Id='02' ")
...
hang:
    ...
    Private Sub LoadReport()
        ...
        LngId = "7095" : CallLngStr() : MuLngRpt = MuLngRpt & "N'" & LngStr & "' As Crl_RptName ,"
        ...
        With Rs
            ...
            .Open("" & SLF & "where Rpt_Id <>'' " & RPT_ID & " " & r & "order by Rpt_Id asc  ", CNN, ADODB.CursorTypeEnum.adOpenForwardOnly, ADODB.LockTypeEnum.adLockReadOnly)
            If .EOF Then MsgBox("ບໍ່ມີຂໍ້ມູນ") : Exit Sub
            If .EOF Then Exit Sub
        End With
        Dim FrmPreview As New FmPreview : FrmClosing()
        Dim Rpt As New CryCashflow_statement
