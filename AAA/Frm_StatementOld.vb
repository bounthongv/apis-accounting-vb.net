Public Class Frm_StatementOld
    ...
    Private Sub LoadFG()
        ...
        Dim s11 As String = "INSERT INTO Ap_Open_PostedLedgers( ac_code , Amount_Dr , Amount_Cr , Amt_Dr , Amt_Cr , Curr   ) " & _
         "select ac_code , sum(amount_dr) , sum(amount_cr) ,  sum(amt_dr) , sum(amt_cr) , Curr  from gen_jn WHERE  1=1 " & CUST_Supp & " " & Cur & " and ac_code='" & TxtAccCode.Text & "'  and gen_jn.date_work   BETWEEN '" & "" & Format(MdStartDate, "yyyy") & "-1-1' AND '" & Format(S, "yyyy-MM-dd") & "'   group BY ac_code , Curr "
        CNN.Execute(s11)
        Dim aa As String
        MuTable = "Open_jn." : Call Find_Company()
        aa = "INSERT INTO Ap_Open_PostedLedgers( ac_code , Amount_Dr , Amount_Cr , Amt_Dr , Amt_Cr  , Curr  ) " & _
         "select ac_code , amount_dr , amount_cr , amt_dr , amt_cr , Curr from Open_jn  WHERE  1=1  " & Cur & " and  ac_code='" & TxtAccCode.Text & "' and date_work='" & "" & Format(MdStartDate, "yyyy") & "-1-1'    "
        CNN.Execute(aa)
        MuTable = "gen_jn." : Call Find_Company()
        aa = " INSERT INTO Ap_PostedLedgers( ac_code , ac_name , date_work , descrip , Open_amount , open_amt , amount_dr , amount_cr , amt_dr , amt_cr , certify , remain , Status ,  curr ) " & _
               "select ac_code ,'' ,date_work, descrip ,0 ,0 ,amount_dr ,amount_cr , amt_dr , amt_cr ,certify, 0 , 0 ,  Curr  from gen_jn    WHERE   1=1 " & CUST_Supp & "  " & Cur & "  and  ac_code='" & TxtAccCode.Text & "' and gen_jn.date_work   BETWEEN '" & Format(MdStartDate, "yyyy-MM-dd") & "' AND '" & Format(MdToDate, "yyyy-MM-dd") & "'   "
        CNN.Execute(aa)
   ...
   " select ac_code, '', '', 'Opening Balance', 0, 0, 0, 0, 0, 0, 0, 0, Open_amount, '', '', '','" & Format(MdStartDate, "yyyy-MM-dd") & "' from Ap_PostedLedgers_Rem group by ac_code,Open_amount  "
    ...
    Public Sub LoadListFG()
        ...
        Call LoadSqlData("SELECT isnull(sum(amt_dr),0) as amt_dr, isnull(sum(amt_Cr),0) as amt_Cr FROM  Ap_PostedLedgers  WHERE  ac_code=N'" & TxtAccCode.Text & "'   ", RSC)
        ...
        Else
            ...
            TxtCredit.Text = "0.00"
        End If
        Call LoadSqlData("SELECT remain FROM  Ap_PostedLedgers  WHERE  ac_code=N'" & TxtAccCode.Text & "'  and Descrip=N'Opening Balance'   ", RSC)
        If RSC.RecordCount > 0 Then
